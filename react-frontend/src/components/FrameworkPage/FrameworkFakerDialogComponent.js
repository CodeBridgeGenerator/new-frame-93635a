...
import frameworkFakerFactory from "./frameworkFakerFactory";
...
const FrameworkFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = frameworkFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("framework").create(elem));
...
export default connect(mapState, mapDispatch)(FrameworkFakerDialogComponent);
