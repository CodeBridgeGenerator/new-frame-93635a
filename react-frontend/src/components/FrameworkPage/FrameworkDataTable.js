...
const FrameworkDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.Template}</p>
...
const pTemplate2 = (rowData, { rowIndex }) => <p >{rowData.environment}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "framework"});
...
<Column field="Template" header="Template" body={pTemplate0} filter={selectedFilterFields.includes("Template")} hidden={selectedHideFields?.includes("Template")}  sortable style={{ minWidth: "8rem" }} />
...
<Column field="environment" header="Environment" body={pTemplate2} filter={selectedFilterFields.includes("environment")} hidden={selectedHideFields?.includes("environment")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Framework Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Framework" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default FrameworkDataTable;