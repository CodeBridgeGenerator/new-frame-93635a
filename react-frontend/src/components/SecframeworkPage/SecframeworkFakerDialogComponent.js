...
import secframeworkFakerFactory from "./secframeworkFakerFactory";
...
const SecframeworkFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = secframeworkFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("secframework").create(elem));
...
export default connect(mapState, mapDispatch)(SecframeworkFakerDialogComponent);
