...
const SecframeworkDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate0 = (rowData, { rowIndex }) => <p >{rowData.secTemp}</p>
    ...
    const paginatorRight = DownloadCSV({ data : items, fileName : "secframework"});
...
<Column field="secTemp" header="SecTemp" body={pTemplate0} filter={selectedFilterFields.includes("secTemp")} hidden={selectedHideFields?.includes("secTemp")}  sortable style={{ minWidth: "8rem" }} />
        ...
        <Dialog header="Upload Secframework Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService />
      </Dialog>

      <Dialog header="Search Secframework" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
...
export default SecframeworkDataTable;